...
<style>
  :root{
    --paper:#D8CAB7; --paper-lo:#D0C1AC;
    --ink:#16233F; --ink-soft:#3B5170; --faint:#B4A48D;
    --accent:#2E5BD8;
  ...
  *{box-sizing:border-box}
  ...
  /* 1 open ---------------------------------------------------------------- */
  /* Line-drawn houses on grained paper. Each outline draws itself rather than
     appearing, which is the whole character of the reference. */
  #s1 svg.street{position:absolute;inset:0;width:100%;height:100%}
  #s1 .stroke{
    fill:none;stroke:var(--line-w);stroke-width:2.6;
    stroke-linecap:round;stroke-linejoin:round;
  }
  /* A message leaving a house. A bubble, not a dot, so it reads as a message
     without anyone having to be told. */
  .bub{
    position:absolute;opacity:0;
    transition:transform 2.9s var(--inout),opacity 2.9s var(--quart);
  }
  .bub svg{display:block;overflow:visible}
  .bub .body{fill:var(--msg);stroke:none}
  ...
  /* Paper grain. The reference has it and the flat fill looks dead without it. */
  #grain{position:absolute;inset:0;pointer-events:none;opacity:.5;
         mix-blend-mode:multiply}
  #stage.night #grain{opacity:0;transition:opacity .8s ease}
...
<div id="fit"><div id="stage">
  <svg id="grain" viewBox="0 0 1920 1080" preserveAspectRatio="none">
    <filter id="gr"><feTurbulence type="fractalNoise" baseFrequency=".9" numOctaves="3"/>
      <feColorMatrix type="saturate" values="0"/></filter>
    <rect width="1920" height="1080" filter="url(#gr)" opacity=".22"/>
  </svg>
  <section class="scene" id="s1"><svg class="street" viewBox="0 0 1920 1080" preserveAspectRatio="none"></svg></section>
...
function clear(){
  ...
  s('s1').querySelectorAll('.bub').forEach(function(e){ e.remove(); });
  s('s1').querySelector('svg.street').innerHTML='';
...
/* 1 ---------------------------------------------------------------------- */
/* Houses are laid out deterministically, so every take is identical, and each
   outline draws itself instead of appearing. Widths and heights vary because a
   row of identical houses reads as a chart, not a street. */
function street(){
  var svg=s('s1').querySelector('svg.street');
  svg.innerHTML='';
  var base=604, left=360, right=1560, n=11, gap=(right-left)/n;
  var houses=[];
  for(var i=0;i<n;i++){
    var w=gap*0.78+((i*29)%11), h=86+((i*47)%42), roof=40+((i*23)%20);
    var x=left+i*gap+(gap-w)/2;
    var d='M'+x+' '+base+' L'+x+' '+(base-h)+' L'+(x+w/2)+' '+(base-h-roof)+
          ' L'+(x+w)+' '+(base-h)+' L'+(x+w)+' '+base;
    houses.push({d:d, peakX:x+w/2, peakY:base-h-roof});
  }
  var ground=document.createElementNS('http://www.w3.org/2000/svg','path');
  ground.setAttribute('class','stroke');
  ground.setAttribute('d','M'+(left-70)+' '+base+' L'+(right+70)+' '+base);
  svg.appendChild(ground);
  draw(ground,1.5,0);
  houses.forEach(function(hh,i){
    var el=document.createElementNS('http://www.w3.org/2000/svg','path');
    el.setAttribute('class','stroke'); el.setAttribute('d',hh.d);
    svg.appendChild(el);
    draw(el,0.85,0.45+i*0.11);
  });
  return houses;
}

function draw(node,dur,delay){
  var len=node.getTotalLength();
  node.style.strokeDasharray=len; node.style.strokeDashoffset=len;
  node.style.transition='stroke-dashoffset '+dur+'s var(--quart) '+delay+'s';
  requestAnimationFrame(function(){ node.style.strokeDashoffset=0; });
}

function playOpen(){
  var sc=s('s1'), houses=street();
  /* Messages leave the houses and drift off unread. */
  for(var k=0;k<8;k++){
    (function(k){
      var hh=houses[(k*3+2)%houses.length];
      var rise=250+((k*67)%170), drift=((k%3)-1)*54;
      at(2.4+k*0.44,function(){
        var b=document.createElement('div');
        b.className='bub';
        b.style.left=(hh.peakX-23)+'px';
        b.style.top=(hh.peakY-58)+'px';
        b.innerHTML="<svg width='46' height='38' viewBox='0 0 34 28'>"+
          "<path class='body' d='M6 0h22a6 6 0 0 1 6 6v10a6 6 0 0 1-6 6H14l-7 6v-6H6a6 6 0 0 1-6-6V6a6 6 0 0 1 6-6z'/>"+
          "</svg>";
        sc.appendChild(b);
        requestAnimationFrame(function(){
          b.style.opacity=.88;
          b.style.transform='translate('+drift+'px,'+(-rise)+'px) scale(.7)';
          setTimeout(function(){ b.style.opacity=0; },1000+((k*53)%400));
        });
      });
    })(k);
  }
...
/* ?s=3&play cues a scene and runs it, which is how these get screenshotted and
   is convenient for cueing a take. */
var q=new URLSearchParams(location.search);
var want=parseInt(q.get('s')||'1',10);
show(want>=1&&want<=5?want:1);
if(q.has('play')) play();
</script>
"""
